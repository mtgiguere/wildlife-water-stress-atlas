...
def test_water_threshold_m_is_positive_number():
    for species, config in SPECIES_CONFIG.items():
        assert isinstance(config["water_threshold_m"], (int, float)), \
            f"{species}: water_threshold_m must be int or float"
        assert config["water_threshold_m"] > 0, \
            f"{species}: water_threshold_m must be positive"


def test_accessible_water_types_is_nonempty_set():
    for species, config in SPECIES_CONFIG.items():
        assert isinstance(config["accessible_water_types"], set), \
            f"{species}: accessible_water_types must be a set"
        assert len(config["accessible_water_types"]) > 0, \
            f"{species}: accessible_water_types must not be empty"


def test_water_type_weights_keys_match_accessible_water_types():
    for species, config in SPECIES_CONFIG.items():
        assert config["water_type_weights"].keys() == config["accessible_water_types"], \
            f"{species}: water_type_weights keys must match accessible_water_types"
    ...
    for species, config in SPECIES_CONFIG.items():
        for water_type, weight in config["water_type_weights"].items():
            assert isinstance(weight, float), \
                f"{species}/{water_type}: weight must be a float"
            assert 0.0 < weight <= 1.0, \
                f"{species}/{water_type}: weight must be between 0 (exclusive) and 1 (inclusive)"


def test_daily_range_m_is_positive_number():
    for species, config in SPECIES_CONFIG.items():
        assert isinstance(config["daily_range_m"], (int, float)), \
            f"{species}: daily_range_m must be int or float"
        assert config["daily_range_m"] > 0, \
            f"{species}: daily_range_m must be positive"


def test_water_dependency_is_valid_string():
    valid_values = {"low", "moderate", "high"}
    for species, config in SPECIES_CONFIG.items():
        assert config["water_dependency"] in valid_values, \
            f"{species}: water_dependency must be one of {valid_values}"
